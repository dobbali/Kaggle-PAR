...
These variable might skew the prediction as well, so Log transformation is applied to these variables to normalize them. 
...
```{r Elasticnetcontinues, include=T, echo=F}
...
The following are the important variables picked by Elastic Net:
...
Elastic Net performs better with predictors which are highly correlated with each other.
...
"MSSubClass","MSZoning","LotShape","LandContour",
                   ...
                   Out of all the predictors PoolQC, Misc Feature, MiscVal, SaleType have 95% of values unchanged. These kind of variables tend to not contribute to the prediction in any way. As 95% of the PoolQC of values are Not avaiables, Pool area for those values is zero. They would influence prediction to very negligible extend. Hence, ignored. Same is the case with Heating, BsmtHalfBat, ExterCond, Street, Utilities, LotConfig etc., They either have more that 95% of the values with a default setting or zero's which will not provide significant impact on predictions as there is no variablity in the values. 
...
We will use the predictor derived from the above methods and built a strong prediction model. 25% of the train set has been used as Validation set while model is build on rest of the 75%. This will help in estimating the RMSE of the final test set. 
...
Clearly the linear Model is overfitting. This might because Linear regression with so many variable tend to overfit sometimes. Moving to XGBoosted regression model to see how it performs and also to extract the strongest predictors.
...
XGBoosted model performs a lot better. Especially with the train set, but still it overfits a bit. 
Looking at the feature importance from the XG boosted model
...
Readjusting the baseline Linear model with predictors obtained from XGboost model above.

```{r, include=F}
frmlaa <- formula("SalePrice ~ OverallQual +  GrLivArea +  TotalBsmtSF +  bathrooms +  YearBuilt +  GarageCars +  YearRemodAdd +  LotArea +  OverallCond +  GarageArea +  FireplaceQu +  KitchenAbvGr + porch +  GarageType + 
GarageYrBlt  + BldgType +  Neighborhood +  CentralAir +  MSZoning + Functional +  PavedDrive +  TotRmsAbvGrd")
...
#xgBoostm <- train(frmlaa, preProcess=c("center","scale"), 
 #                 data = t.train, method="xgbTree",
 #                 metric="RMSE",
 #                 tuneGrid=CARET.TUNE.GRID
                  #)
...
#write.csv(dt.result, 'Result_lmmm_adj.csv', row.names = FALSE)
...
With the new set of predictors obtained from XGboost, this new baseline linear model performed well if we use R2 as indicator but this model is overfitting as the differnce between training and validation set it quite hight.
It can be concluded that the predictive power is not that great and hence not performing better than XGboost on validation test data set.
...
Difference between model performance is not so significant but results of Gradient Boosted Model are not as good as the XG Boosted Model on the test set, but outperforms it on the train set. It has defintely better performance than the baseline linear model. Even though bias is reduc
...
cat("RMSE of the Random forest model on train set : ", rmse(exp(t.train$SalePrice)-1,exp(predict(ranger_mod, newdata = t.train))-1))

cat("RMSE of the Random forest model on test set : ", rmse(exp(t.test$SalePrice)-1,exp(predict(ranger_mod, newdata = t.test))-1))

cat("Rsquared value of the Random forest Model : ",ranger_mod$results$Rsquared )

```

Random forest regressor has the lowest RMSE on train set compared with other models but it the worst performer when implemented on test set. The model is being overfitted a lot and  R2 value is also low compared to XGboost.
...
Even though Ensemble methods take lot of computation power they are well know to achieve enhanced predictive performance. Here, predicitions of Gradient boost , XGboost and Random Forest methods are combined together


```{r, include=T,echo=F, warning=F}

cat("RMSE of the Ensembled(Linear Regression) model on train set : ", rmse(exp(t.train$SalePrice)-1,exp(predict(ce, newdata = t.train))-1))

cat("RMSE of the Ensembled(Linear Regression) model on test set : ", rmse(exp(t.test$SalePrice)-1,exp(predict(ce, newdata = t.test))-1))

#cat("Rsquared value of the Ensembled Model : ",ce$results$Rsquared )


```

As expected, Ensemble method worked well compared to individual models. RSME values of train and test sets are pretty close to each other. Here data sets are equal in size and RSME values of train and test sets being proportional to each other indicates that the model is reliable.  

From the summary of ensembled model, it is clear that XGboost has been given maximum weight. Even while creating models using individual algorithms, it has been found that XGboost performed well. One more way to verify this fact is that the method performed well would be the fit for individual model on the RMSE is also low for XGboost.
So, the results are consisent with each other with high bias and low variance and ensemble methods has resulted in model with right weightage. 

### Model Ensemble with Neural Net
...
cat("RMSE of the Ensembled(Neural Net) model on train set : ", 
rmse(exp(dt.train$SalePrice)-1,exp(predict(l1_nnet_mdl))-1))

```

All individual predictions from the Gradient Boost Method, Random Forest and XGBoosted model are assembled into a data frame. On top of these predictions a Neural net Model is applied to stack the above model.
Neural net assign weights to the predictions, in turn giving weights to the individual models. 
This way the best predictions from each model is picked. We created the final predictions from this model.

GBM + RF + XGBoosted -> Preds -> NN -> Final Preds
