...
dt.test$GarageQual <- ifelse(is.na(dt.test$GarageQual),"NoGarage", dt.test$YesGarageQual)
...
frmla <- "SalePrice ~ "
for(name in numCols.tr){
  if(name != "Id" &&  name != "SalePrice"){
    frmla <- paste(frmla  ,"+" , name)
  }
}

lasso_mod <- train(SalePrice ~LotFrontage + LotArea + OverallQual + OverallCond + YearBuilt + YearRemodAdd + MasVnrArea + BsmtUnfSF + TotalBsmtSF  + GrLivArea + bathrooms + totalRoom + Fireplaces  + GarageArea + porch + YrSold, data = dt.train,
